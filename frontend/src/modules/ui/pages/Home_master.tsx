...
const Home_master: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        await Promise.all([healthz(), bootstrap('master')])
  ...
  const handleStart = async () => {
    ...
    // Si ya hay set para master, simplemente continuamos
    if (setId) {
      nav(`/level/${LEVEL_KEY}`)
      setLoading(false)
      return
    ...
    try {
      ...
      // Lanzamos /start para master en paralelo
      startLevel(LEVEL_KEY)
        ...
        .catch(e => {
          console.error('Error en startLevel (master):', e)
          setIntroError('No se pudo iniciar la simulación')
        })
    } catch (e: any) {
      console.error('Error cargando contexto del nivel master:', e)
      setIntroError(
        e?.message || 'No se pudo cargar la introducción del nivel master'
      )
      setShowIntro(false)
      setStatus(
        `Error: ${
  ...
  const handleDeleteProgress = async () => {
    ...
    if (!window.confirm('¿Seguro que quieres eliminar tu progreso del nivel master?')) return
  ...
  return (
    ...
    >
      {/* Fondo (puedes cambiarlo por uno específico de master si quieres) */}
      <img
      ...
      >
        <h2 style={{ margin: 0 }}>Inicio · Nivel master</h2>
     ...
     {/* 🌟 NUEVO: Continuar master solo si existe set en master */}
      {!hasSeniorOpen && !loading && (
        ...
        {setId && (
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Eliminando…' : 'Eliminar progreso master'}
          </button>
        )}
      </div>
      {/* Overlay INTRO */}
      {showIntro && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Introducción del nivel master"
        >
          <div
            className="card"
            style={{
                  ...
                  {isLastStep && (
                    <>
                      {!startReady && (
                        ...
                        >
                          GhostCoder está sincronizando tu estacion de trabajo del nivel master. Esto puede tardar unos segundos…
                        </p>
                      )}
                      ...
                      {startReady && (
                        ...
                        >
                          ✔ Configuración completada — Los lideres han configurado tu estacion de trabajo con exito.
                        </p>
                      )}
                    </>
                  )}
